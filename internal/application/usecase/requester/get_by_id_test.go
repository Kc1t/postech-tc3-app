...
func TestGetRequester_Execute_Success(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	expected := entities.ReconstituteRequester("", "João", "123", "j@j.com", "11999", time.Now(), time.Now())
	repo := mocks.NewMockRequesterRepository(ctrl)
	repo.EXPECT().FindByID(gomock.Any(), "cust-1").Return(expected, nil)

	uc := NewGetRequester(repo)
...
func TestGetRequester_Execute_NotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
