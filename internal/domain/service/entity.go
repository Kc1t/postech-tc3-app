...
func New(name, description string, price float64, durationMin int) *Service {
	now := time.Now()
	return &Service{
		name:        name,
...
func Reconstitute(id, name, description string, price float64, durationMin int, createdAt, updatedAt time.Time) *Service {
	return &Service{
		id:          id,
		name:        name,
		description: description,
